const questions = [
    {
      id: 1,
      title: 'What are the Five Pillars of Islam?',
      info:
        'The Five Pillars of Islam are the five basic acts of worship that are considered mandatory for Muslims. They are the declaration of faith (Shahada), prayer (Salah), giving of alms (Zakat), fasting (Sawm), and pilgrimage to Mecca (Hajj)'
    },
    {
      id: 2,
      title: 'Are Muslims allowed to listen to music?',
      info:'In Islam, the permissibility of listening to music is a topic of debate among scholars. Some scholars consider certain types of music to be permissible, while others believe that all music is prohibited. The key factors that influence this debate include the content and purpose of the music, as well as its potential impact on an individuals behavior and spirituality.'
    },
    {
      id: 3,
      title: 'What is the difference between Sunni and Shia Islam?',
      info:'The main difference between Sunni and Shia Islam lies in their beliefs regarding the rightful successor to the Prophet Muhammad Sunni Muslims believe that the first four caliphs, including Abu Bakr, Umar, Uthman, and Ali, were the rightful successors to the Prophet Muhammad. They make up the majority of the Muslim population while the Shia Muslims, on the other hand, believe that Ali, the cousin and son-in-law of the Prophet Muhammad, was the rightful successor. They believe in the concept of Imamah, which asserts that leadership should have passed directly to Ali and his descendants'
    },
    {
      id: 4,
      title: 'What is the concept of Jihad in Islam?',
      info:'The concept of Jihad in Islam is often misunderstood and misrepresented. Jihad, in its literal meaning, translates to "struggle" or "striving." In Islamic teachings, Jihad encompasses various forms of struggle, including the internal struggle against ones own desires and temptations, the struggle to improve society and uphold justice, and, when necessary, the physical defense of oneself and the community.',
    },
    {
      id: 5,
      title: 'What do muslims believe about trinity?',
      info: 'Muslims don’t believe in the Trinity. The Quran denies it in Surah 4:171: “O People of the Scripture. do not commit excess in your religion or say about Allah except the truth. The Messiah, Jesus, the son of Mary, was but a messenger of Allah and His word which He directed to Mary and a soul [created at a command] from Him. So, believe in Allah and His messengers. And do not say, “Three”; desist – it is better for you. Indeed, Allah is but one God. Exalted is He above having a son. To Him belongs whatever is in the heavens and whatever is on the earth. And sufficient is Allah as Disposer of affairs.'
    },
  ]
  export default questions